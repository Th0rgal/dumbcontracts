...
27. `Compiler/Proofs/SemanticBridge.lean` states the target EDSL≡compiled-IR theorems for SimpleStorage (store, retrieve) and attempts full discharge via direct `simp` unfolding of both EDSL and IR execution — no `sorry` in the SimpleStorage proofs.
