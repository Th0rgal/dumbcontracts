...
1. `execYulStmts_genParamLoads_erases_vars`: executing the genParamLoads prefix on any
   ...
   shadow all prior vars with the same names, and calddataload doesn't depend on vars).
2. `yulBody_from_state_eq_yulBody`: composes (1) with the result-wrapping equivalence.
-/

/-- **Core gap lemma**: For an IR function whose body starts with calddataload let-bindings
...
`let name := calddataload(offset)`, any prior binding of `name` is removed. The
`calddataload` evaluation depends only on `state.calldata` (not vars), so both starting
...
1. **`execYulStmts_paramState_eq_emptyVars`** (1 sorry)
   ...
   - Why it holds: genParamLoads-generated `let name := calddataload(offset)` statements
     re-bind every parameter from calldata. `YulState.setVar` uses filter-then-prepend,
     erasing any prior binding of the same name. `calddataload` doesn't depend on vars.
