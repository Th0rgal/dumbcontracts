...
composition. The next compiled-side substep is now stated explicitly too: prove
that `interpretIRWithInternals` is a conservative extension of legacy
`interpretIR` on the legacy-compatible external-body Yul subset generated by
today's supported fragment, then retarget the theorem stack to that richer IR
target. The compiled-side blocker is tracked in
