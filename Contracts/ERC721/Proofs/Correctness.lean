...
theorem balanceOf_preserves_state (s : ContractState) (addr : Address) :
    ((Contracts.MacroContracts.ERC721.balanceOf addr).runState s) = s := by
  rw [Contracts.MacroContracts.ERC721.balanceOf]
  change ((Verity.bind (getMapping Contracts.MacroContracts.ERC721.balances addr) Verity.pure).runState s) = s
...
theorem ownerOf_preserves_state (s : ContractState) (tokenId : Uint256) :
    ((Contracts.MacroContracts.ERC721.ownerOf tokenId).runState s) = s := by
  cases h_owner : (s.storageMapUint Contracts.MacroContracts.ERC721.owners.slot tokenId != 0) <;>
    simp [Contracts.MacroContracts.ERC721.ownerOf, getMappingUint, Contract.runState, Verity.bind, Bind.bind,
...
theorem getApproved_preserves_state (s : ContractState) (tokenId : Uint256) :
    ((Contracts.MacroContracts.ERC721.getApproved tokenId).runState s) = s := by
  cases h_owner : (s.storageMapUint Contracts.MacroContracts.ERC721.owners.slot tokenId != 0) <;>
    simp [Contracts.MacroContracts.ERC721.getApproved, getMappingUint, Contract.runState, Verity.bind, Bind.bind,
      require, Pure.pure, Verity.pure, h_owner]
...
theorem isApprovedForAll_preserves_state (s : ContractState) (ownerAddr operator : Address) :
    ((Contracts.MacroContracts.ERC721.isApprovedForAll ownerAddr operator).runState s) = s := by
  simp [Contracts.MacroContracts.ERC721.isApprovedForAll, getMapping2, Contract.runState, Verity.bind, Bind.bind]
...
theorem setApprovalForAll_is_balance_neutral_holds
    ...
    setApprovalForAll_is_balance_neutral s ((Contracts.MacroContracts.ERC721.setApprovalForAll operator approved).runState s) := by
