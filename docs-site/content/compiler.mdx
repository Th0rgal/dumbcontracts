...
For proof-strict builds, deny flags lock down the trust surface: `--deny-linear-memory-mechanics`, `--deny-event-emission`, `--deny-axiomatized-primitives`, `--deny-proxy-upgradeability`, `--deny-local-obligations`. See [Glossary](/glossary) for one-line definitions.
