...
| `getStorageAddr` | `StorageSlot Address → Contract Address` |
| `setStorageAddr` | `StorageSlot Address → Address → Contract Unit` |
| `getMapping` | `StorageSlot (Address → Uint256) → Address → Contract Uint256` |
| `setMapping` | `StorageSlot (Address → Uint256) → Address → Uint256 → Contract Unit` |
| `getMappingUint` | `StorageSlot (Uint256 → Uint256) → Uint256 → Contract Uint256` |
| `setMappingUint` | `StorageSlot (Uint256 → Uint256) → Uint256 → Uint256 → Contract Unit` |
| `getMapping2` | `StorageSlot (Address → Address → Uint256) → Address → Address → Contract Uint256` |
| `setMapping2` | `StorageSlot (Address → Address → Uint256) → Address → Address → Uint256 → Contract Unit` |
...
Elaborates to `Expr.keccak256`; lowers to Yul `keccak256(offset, size)`. Treated as axiomatized primitive in the current proof stack (`keccak256_memory_slice_matches_evm`). Use `--deny-axiomatized-primitives` for proof-strict runs.
...
First-class forms exist in the compilation model (`Expr.call`, `Expr.staticcall`, `Expr.delegatecall`) but are not yet fully modeled by the proof interpreters. `delegatecall` is also isolated as a proxy/upgradeability boundary — use `--deny-proxy-upgradeability`. Linear-memory forms (`Expr.mload`, `Stmt.mstore`, `Stmt.calldatacopy`, `Stmt.returndataCopy`, `Expr.returndataOptionalBoolAt`) compile but are partially modeled — use `--deny-linear-memory-mechanics`.
