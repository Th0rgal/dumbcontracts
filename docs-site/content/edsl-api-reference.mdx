...
Each entry lowers into `spec.constructor.localObligations` or `FunctionSpec.localObligations`, shows up in `--trust-report`, and participates in `--deny-local-obligations`. If a constructor or function uses direct assembly-shaped primitives without any `local_obligations [...]`, compilation fails closed.
...
`verity_contract` accepts tuple-shaped local sugar for the static tuple cases supported by the compilation model:
...
For executable Lean definitions generated by `verity_contract`, tuple-typed parameters stay tuple-shaped as right-nested Lean products instead of being erased to `Unit`. The same ABI-flattened component aliases are also injected into the executable body, so a parameter such as `pair : Tuple [Uint256, Uint256]` can be referenced directly as `pair_0` / `pair_1` inside the contract body without dropping back to manual destructuring.

## Stateless Contracts

`verity_contract` accepts an empty `storage` block when a contract is purely functional or only depends on ambient values such as `msg.sender`:
...
`verity_contract` can declare linked external functions directly and use them from `externalCall` expressions:
...
`verity_contract` exposes the EIP-1153 transient storage builtins directly:
...
`verity_contract` accepts custom error declarations and threads them into the generated `CompilationModel`/ABI surface:
...
These declarations populate `CompilationModel.errors`, and the macro lowers guarded and unconditional custom-error exits to `Stmt.requireError` / `Stmt.revertError`. That keeps `#check_contract`, ABI generation, and the Yul compiler on the same custom-error surface authored in the macro contract.
...
If the contract already defines a same-named function with the matching arity, the macro rejects the direct ERC-20 helper form instead of silently reinterpreting it. That keeps `balanceOf` / `safeTransfer` helper syntax reserved for ECM lowering and avoids ambiguous model generation.
...
When the low-level form is `delegatecall`, the trust report also isolates it as a dedicated proxy / upgradeability boundary; archive `--trust-report` and use `--deny-proxy-upgradeability` for proof-strict runs (see issue `#1420`).
...
When a tiny unsafe/assembly-shaped escape hatch is unavoidable, `FunctionSpec.localObligations` / `ConstructorSpec.localObligations` let the contract author attach a named local refinement obligation to that exact usage site. The trust report records these under `localObligations` and `proofStatus.*.localObligations`, and `--deny-local-obligations` fails closed unless every such obligation has been discharged to `proved`.
