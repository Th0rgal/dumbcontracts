...
Compilation correctness is generic: the framework's `Compiler/TypedIRCompilerCorrectness.lean` and `Compiler/Proofs/IRGeneration/Contract.lean` cover the supported fragment with no per-contract bridge proofs needed. See [Verification Status](/verification).
