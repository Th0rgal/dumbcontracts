...
mkdir -p Verity/Specs/NameRegistry

# Create the three required files
touch Verity/Specs/NameRegistry/Spec.lean
touch Verity/Specs/NameRegistry/Invariants.lean
touch Verity/Specs/NameRegistry/Proofs.lean

# Create EDSL implementation
touch Verity/Examples/NameRegistry.lean
...
- Anyone can look up a name by calling `getName(address) → string`
...
Open `Verity/Specs/NameRegistry/Spec.lean`:
...
namespace Verity.Specs.NameRegistry
...
def setName_spec (name : String) (s s' : ContractState) : Prop :=
  ...
  s'.storageMap 0 (addressToKey s.sender) = stringToStorageValue name ∧
  -- Mapping storage is only changed at this key
  (∀ addr : Address, addr ≠ s.sender →
    s'.storageMap 0 (addressToKey addr) = s.storageMap 0 (addressToKey addr)) ∧
...
def setName_getName_roundtrip (name : String) (s : ContractState) : Prop :=
  ∀ s_after_set : ContractState,
    setName_spec name s s_after_set →
    getName_spec s.sender name s_after_set

end Verity.Specs.NameRegistry
```
...
Open `Verity/Specs/NameRegistry/Invariants.lean`:
...
Open `Verity/Examples/NameRegistry.lean`:
...
/-
  ...
  Demonstrates:
  - Mapping storage (address → string)
  - String handling
...
end Verity.Examples.NameRegistry
```
...
lake build Verity.Examples.NameRegistry
```
...
Open `Verity/Specs/NameRegistry/Proofs.lean`:
...
```lean
-- NameRegistry: Address → Name mapping
...
import Verity.Examples.NameRegistry
...
open Verity
open Verity.Examples.NameRegistry
...
grep -r "sorry" Verity/Specs/NameRegistry/
```
 ...
 * @notice Tests that verify EDSL ≡ Yul ≡ EVM execution for NameRegistry
 */
...
```markdown
| NameRegistry | ✅ | 3/3 (100%) | Address-to-name mapping | 500 |
```
...
**Solution**:
```bash
grep -r "sorry" Verity/Specs/YourContract/
```
...
You've now:
- ✅ Written formal specifications
- ✅ Implemented a contract in the EDSL
- ✅ Proven correctness properties
- ✅ Compiled to Yul and bytecode
- ✅ Tested against the full stack
...
**Welcome to verified smart contract development! 🎉**
