...
When you define a contract using the `verity_contract` macro in `Contracts/<Name>/<Name>.lean`, the framework automatically provides compilation correctness via the generic typed-IR theorem in `Verity/Core/Free/TypedIRCompilerCorrectness.lean`. This theorem covers a grammar of 36+ supported statement fragments, so no per-contract compilation proofs are needed for standard patterns.
