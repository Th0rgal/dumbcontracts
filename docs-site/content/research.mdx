...
**Usage**:
```bash
lake build compile-new    # Build compiler
...
**Plan**:
- Build EDSL interpreter in Lean (execute contracts on abstract state)
- Generate random transactions from contract types
- Run same transactions on: (a) EDSL interpreter, (b) compiled Yul on EVM
- Assert identical results: storage changes, reverts, return values
- Target: 10,000+ tests per contract (70k+ total)

**Success**: Zero mismatches across all random transactions.
...
6. Differential testing (70k+ random tests)
7. Property extraction (252 theorems → tests)
8. Compiler verification (formal correctness proof)
