...
- **Layer 2 (ContractSpec → IR)**: Per‑function preservation theorems complete for all 7 contracts; contract‑level dispatch generalization is underway (SimpleStorage done).
...
- ✅ Contract-level dispatch preservation exists for SimpleStorage (selectors wired to transactions).
