...
See [VERIFICATION_STATUS.md](https://github.com/Th0rgal/verity/blob/main/docs/VERIFICATION_STATUS.md) for the live proof-status snapshot.

- **Contract-specific specification proofs**: Complete for all 8 verified contracts (7 compiler-spec contracts + ReentrancyExample).
- **Layer 2 (CompilationModel → IR)**: Partial generic coverage. A supported-statement theorem exists; whole-contract preservation still relies on contract-specific bridges. The generic `CompilationModel.compile` theorem is tracked in [#1510](https://github.com/Th0rgal/verity/issues/1510).
- **Layer 3 (IR → Yul)**: Complete — all 8 statement equivalence proofs + universal dispatcher proven.
...
- **Yul↔EVM bridge**: Add a minimal semantic bridge lemma or mechanized model.
