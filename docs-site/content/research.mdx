...
- **Layer 3 (IR -> Yul)**: In progress (core IR/Yul semantics aligned, runtime preservation theorem in place, smoke tests assert revert preservation across IR and Yul, and `YulGeneration/Equivalence.lean` now includes aligned-Yul helpers plus `calldataload` evaluation lemmas for offset 0, <4, and >=4).
...
- Guard the manifest against drift: keep `test/property_manifest.json` synchronized with proofs via `scripts/check_property_manifest_sync.py`.
  - Property tag parsing now accepts suffixes like `Property 12b: name (fuzz test)` to keep notes in test files without breaking coverage checks.
