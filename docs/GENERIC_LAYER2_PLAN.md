...
- the remaining helper blocker is now pinned down more precisely in
  ...
  the repo still lacks a conservative-extension lemma from legacy `interpretIR`
  to `interpretIRWithInternals` on the legacy-compatible external-body Yul subset
  generated by today's supported fragment,
...
- internal helper compositional reuse:
  ...
  `IRContract.internalFunctions`, so the remaining work is first proving those
  semantics conservatively extend legacy `interpretIR` on the legacy-compatible
  external-body Yul subset generated by today's supported fragment, then
  retargeting the proof stack to those semantics and consuming helper-summary
  soundness/rank evidence
