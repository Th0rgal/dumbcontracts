...
13. `solc-compat-v0` now carries compatibility-only object rewrites:
   ...
   `solc-compat-materialize-increment-uint256-helper` is implemented and tested, but intentionally kept out of default bundle activation while deploy/runtime wrapper-shape interactions are being hardened.
