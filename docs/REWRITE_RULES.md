...
8. Rewrite bundles are now explicit and versioned (`foundation`, `solc-compat-v0`), with bundle selection propagated by `PatchPassConfig.rewriteBundleId`.
   ...
   - `solc-compat-rewrite-accrue-interest-checked-arithmetic`: rewrites selected runtime `accrueInterest` arithmetic expressions to Solidity-style `checked_add_uint256` / `checked_mul_uint256` / `checked_div_uint256` calls and materializes those helpers only when referenced and absent.
