...
3. Add compositional helper-call proof reuse across callers.
   ...
   More precisely, `artifacts/layer2_boundary_catalog.json` now records that callers still derive generic body proofs through the helper-free `SupportedStmtList` witness, summary-soundness evidence is not yet consumed by the helper-aware body theorem, and the public Layer 2 theorems still quantify over legacy `execIRFunction` / `interpretIR` rather than the new helper-aware IR target. The immediate compiled-side substep is now explicit too: first prove `interpretIRWithInternals` is a conservative extension of `interpretIR` on the legacy-compatible external-body Yul subset generated by today's supported fragment, then thread summary soundness plus the decreasing helper-rank measure through the body/IR preservation lemmas while retargeting those lemmas to `execIRFunctionWithInternals` / `interpretIRWithInternals` so `calls.helperCompatibility` can disappear. The compiled-side blocker is tracked in [#1638](https://github.com/Th0rgal/verity/issues/1638).
