...
- CI now runs `check_yul_compiles.py` + `check_gas_model_coverage.py` on `generated/yul-patched` as part of Issue #582 fail-closed hardening for patch-enabled output, including filename-set parity checks against baseline Yul output
- CI now runs a dedicated Foundry patched-Yul smoke gate (`DIFFTEST_YUL_DIR=generated/yul-patched`) so differential/property harnesses execute against patch-enabled output
