...
3. Add compositional helper-call proof reuse across callers.
   Current unblocker: `Compiler/Proofs/IRGeneration/SourceSemantics.lean` now has a spec-aware helper execution target (`evalExprWithHelpers`, `execStmtListWithHelpers`, `interpretInternalFunctionFuel`), and the public Layer 2 theorems now already target that helper-aware semantics through the canonical `SupportedSpec.helperFuel` bound. `calls.helpers` carries both a reusable `InternalHelperSummaryContract` slot and an explicit decreasing helper-rank measure for direct callees. Compatibility lemmas still prove that this helper-aware semantics collapses to the legacy helper-free semantics on the current fragment, so the next structural blocker is specifically to connect the summary contract interface and rank measure to the source/body preservation lemmas so `calls.helpers.legacySurfaceClosed` can disappear instead of remaining the final gate.
