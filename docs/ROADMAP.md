...
3. Add compositional helper-call proof reuse across callers.
   ...
   More precisely, `artifacts/layer2_boundary_catalog.json` now records that callers still derive generic body proofs through the helper-free `SupportedStmtList` witness, the current generic body theorem still targets helper-free `SourceSemantics.execStmtList`, and `execIRFunction` does not yet model internal helper call composition.
