...
- Spec correctness placeholder: `Compiler/Proofs/SpecCorrectness/SafeMultisigBase.lean`
  - Note: compiler spec constructor is currently a no-op to match proxy-style init.
...
3. Implement the base contract in the DumbContracts EDSL.
  ...
  - Added placeholder `setFallbackHandler` entrypoint restricted to self-calls
    (and preventing handler = Safe).
  - Added placeholder `changeThreshold` entrypoint restricted to self-calls
    with minimal bounds checks (0 < threshold ≤ ownerCount).
