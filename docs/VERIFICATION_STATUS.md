...
- the machine-readable boundary catalog now makes that blocker exact rather than implicit: callers still reach the body theorem through the helper-free `SupportedStmtList` witness, the helper-aware body theorem does not yet consume helper-summary soundness/rank evidence end to end, and the public theorem stack still targets legacy `execIRFunction` / `interpretIR` even though `IRInterpreter.lean` now defines helper-aware compiled-side targets (`execIRFunctionWithInternals`, `interpretIRWithInternals`) that can resolve `IRContract.internalFunctions`. The immediate compiled-side substep is now explicit too: the repo still needs a conservative-extension lemma from `interpretIR` to `interpretIRWithInternals` on the legacy-compatible external-body Yul subset generated by today's supported fragment before those public theorems can be retargeted safely. The compiled-side blocker is tracked in [#1638](https://github.com/Th0rgal/verity/issues/1638)
