...
- **`check_patch_gas_delta.py`** - Compares baseline vs patch-enabled static gas reports, enforces median/p90 non-regression thresholds, and requires at least one measurable contract-level improvement
...
# Check patch-enabled static gas deltas (median/p90 non-regression + at least one improvement)
